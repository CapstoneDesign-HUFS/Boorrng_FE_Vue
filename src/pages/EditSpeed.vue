    ...
    <div class="phone-frame">
      <div class="content">      
        ...
        <div class="button-container">
          ...
          <button class="next-button" @click="goToNext">저장</button>
        </div>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        age: '20',
        gender: '여성',
        minSelectedSpeed: 1.23, // 기본값 설정 (걷기)
        maxSelectedSpeed: 1.45, // 기본값 설정 (빨리걷기)
        showModal: false,
        modalMessage: '',
        speedOptions: [
          { name: '걷기', value: 1.23 },
    ...
    created() {
      // 라우터로부터 데이터를 받는 경우 (실제 구현시)
      // const minSpeed = this.$route.query.minSpeed ? parseFloat(this.$route.query.minSpeed) : 1.23;
      // const maxSpeed = this.$route.query.maxSpeed ? parseFloat(this.$route.query.maxSpeed) : 1.45;
      // this.minSelectedSpeed = minSpeed;
      // this.maxSelectedSpeed = maxSpeed;
      
      // API로부터 데이터를 받는 경우 (실제 구현시)
      // this.fetchUserSettings();
    },
    methods: {
      // 기존 설정 데이터를 가져오는 메서드 (실제 구현시 사용)
      // fetchUserSettings() {
      //   // API 호출 예시
      //   // axios.get('/api/user/settings').then(response => {
      //   //   this.minSelectedSpeed = response.data.minSpeed;
      //   //   this.maxSelectedSpeed = response.data.maxSpeed;
      //   // }).catch(error => {
